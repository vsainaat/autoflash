...
public class Server extends autoflash.rpc.slice._ClientServiceDisp {
	 ...
	 * For Power Station
	 */
	public double rentBattery(String stationID, String vehicleID, String batteryID, double amount, Ice.Current current) {		
	...
	public double returnBattery(String stationID, String vehicleID, String batteryID, double amount, Ice.Current current) {
	...
	public void moveBatteryToStation(String stationID, String batteryID, Ice.Current current) {
		log("Station[%s], move in battery[%s].", stationID, batteryID);
		service_.moveBatteryToStation(stationID, batteryID);
	}

	public void moveBatteryFromStation(String stationID, String batteryID, Ice.Current current) {
		log("Station[%s], move out battery[%s].", stationID, batteryID);
		service_.moveBatteryFromStation(stationID, batteryID);
	}

	public void reportDamagedBattery(String stationID, String batteryID, Ice.Current current) {
		log("Station[%s], report damaged battery[%s].", stationID, batteryID);
		service_.reportDamagedBattery(stationID, batteryID);
	}

	public String registerVehicle(String stationID, VehicleInfo info, Ice.Current current) {
	 ...
	 * For Depot
	 */
	public void moveBatteryToDepot(String depotID, String batteryID, Ice.Current current) {
		log("Depot[%s], move in battery[%s].", depotID, batteryID);
		service_.moveBatteryToDepot(depotID, batteryID);
	}

	public void moveBatteryFromDepot(String depotID, String batteryID, Ice.Current current) {
		log("Depot[%s], move out battery[%s].", depotID, batteryID);
		service_.moveBatteryFromDepot(depotID, batteryID);
	}

	public void charge(String depotID, String batteryID, double currentAmount, double useAmount, Ice.Current current) {
		log("Depot[%s] charge battery[%s].", depotID, batteryID);
		service_.charge(depotID, batteryID, currentAmount, useAmount);
	}

	public void discard(String depotID, String batteryID, Ice.Current current) {
	...
	public String purchase(BatteryInfo info, Ice.Current current) {
		log("Purchase new battery.");
		return service_.purchase(info);
	}
	...
	public Activity[] queryBatteryActivities(String batteryID, int start, int end, Ice.Current current) {
		log("Query battery activies.");
		return service_.queryBatteryActivities(batteryID, start, end);
	}

	public Activity[] queryStationActivities(String stationID, int start, int end, Ice.Current current) {
		log("Query station activies.");
		return service_.queryStationActivities(stationID, start, end);
	}

	public Activity[] queryDepotActivities(String stationID, int start, int end, Ice.Current current) {
	...
	public void unregisterStation(String stationID, Ice.Current current) {
		log("Unregister station[%s].", stationID);
		service_.unregisterStation(stationID);
	}

	public void setStation(String stationID, StationInfo info, Ice.Current current) {
		log("Set station[%s].", stationID);
		service_.setStation(stationID, info);
	}

	public void unregisterDepot(String depotID, Ice.Current current) {
		log("Unregister station[%s].", depotID);
		service_.unregisterDepot(depotID);
	}

	public void setDepot(String depotID, DepotInfo info, Ice.Current current) {
