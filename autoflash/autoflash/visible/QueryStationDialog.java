...
public class QueryStationDialog extends Dialog {
	private static autoflash.rpc.slice.ClientServicePrx service_ = ClientProxy.service();

	public QueryStationDialog(Shell parent) {
		super(parent);
	...
	public void open() {
		...
		dialog.setText("查询加能站");
		dialog.setLayout(new GridLayout(2, false));
		
		GridData gd = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
		String[] labTexts = {"加能站ID","最小左边界","最大右边界","最小上边界", "最大下边界", "最小容量", "最大容量", "所有者"};
		String[] textTexts = {"", "-9999", "9999", "-9999", "9999", "-1", "99999999", ""};
		final Text[] texts = new Text[8];
		for (int i = 0; i < 8; ++i) {
		...
		button.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent event) {
				...
				c.owner = texts[7].getText();
				c.maxCapacity = Integer.parseInt(texts[6].getText());
				c.minCapacity = Integer.parseInt(texts[5].getText());
				c.status = OpenStatus.convert(comboOpen.getSelectionIndex());
				try {
					StationInfo[] ss = service_.queryStations(c);
					table.removeAll();
